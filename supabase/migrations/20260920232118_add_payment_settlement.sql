-- ══════════════════════════════════════════════════════════════════════════════
-- PAYMENT SETTLEMENT — public.settle_payment_order
--
-- One atomic entry point that turns a cryptographically verified provider
-- callback into settled money, granted entitlements and an issued invoice.
--
-- The caller (webhook-paymob) proves AUTHENTICITY by verifying the provider's
-- HMAC. This function proves BUSINESS TRUTH. Everything it decides comes from
-- the frozen canonical order plus the provider's SIGNED facts — never from the
-- browser, never from unsigned callback fields such as merchant_order_id or
-- extras, and never from a caller-supplied price.
--
-- Atomicity: the order update, the entitlement projection, the invoice and the
-- fulfilment marker all commit together or not at all. There is deliberately no
-- repair path: a replayed callback that finds an incomplete settlement reports
-- it and writes nothing, so the underlying fault stays visible.
--
-- Adds ONE new object. No existing table, column, constraint, trigger, policy
-- or function is altered.
-- ══════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- ── 12. Fulfilment marker — only after entitlements and invoice succeeded ──
  update public.payment_orders
...
comment on function public.settle_payment_order(
  ...
  'into entitlements, issues exactly one invoice and marks fulfilment. Returns a '
  'jsonb outcome; performs no repair of an already-settled order.';

-- ── Privileges ───────────────────────────────────────────────────────────────
-- Settlement is a server-side operation. No browser role may ever reach it.
