...
export function buildProfessionalTemplate(cv: CvJsonV1): Paragraph[] {
  const language = "en";
  ...
  if (cv.summary.trim() !== "") {
    sections.push(renderSectionTitle("Professional Summary", language));
    sections.push(...renderSummary(cv));
  }

  if (hasCoreCompetencies(cv)) {
    sections.push(renderSectionTitle("Core Competencies", language));
    sections.push(...renderCoreCompetencies(cv));
  }

  if (hasItems(cv.experience)) {
    sections.push(renderSectionTitle("Professional Experience", language));
    sections.push(...renderExperienceItems(cv.experience, language));
  }

  if (hasItems(cv.education)) {
    sections.push(renderSectionTitle("Education", language));
    sections.push(...renderEducationItems(cv.education, language));
  }

  if (hasItems(cv.certifications)) {
    sections.push(renderSectionTitle("Certifications", language));
    sections.push(...renderCertifications(cv.certifications, language));
  }

  if (hasItems(cv.projects)) {
    sections.push(renderSectionTitle("Projects", language));
    sections.push(...renderProjects(cv.projects, language));
  }

  if (hasItems(cv.languages)) {
    sections.push(renderSectionTitle("Languages", language));
