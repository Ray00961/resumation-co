...
// Constant-time string comparison — prevents timing-based token oracle attacks.
...
Deno.serve(async (req) => {
  ...
  // wt  = HMAC-SHA256(WISHMONEY_SECRET, "${tid}:${fid}") — per-transaction auth token
  ...
  // ── GATE 1: WishMoney platform test pings (zero params) — acknowledge silently ──
  // These are direct probes to the URL with no query params at all, not real callbacks.
  if (!tid && !fid) {
    return new Response("ok", { status: 200 });
  }

  // ── GATE 2: HMAC Webhook Token Verification ──────────────────────────────────
  // Every legitimate callback URL was generated by create-cv-order with a token:
  //   wt = HMAC-SHA256(WISHMONEY_SECRET, "${tid}:${fid}")
  // This ties the token cryptographically to the exact transaction + form.
  // An attacker without WISHMONEY_SECRET cannot forge a valid token.
  // This check runs BEFORE any database read or write — nothing is processed
  // unless the token is present and correct.

  if (!WISHMONEY_SECRET) {
    // Server misconfiguration — fail closed, do not process anything
  ...
  // ── Verification passed — request is authentic ───────────────────────────────

  // ── GATE 3: Required param guard (defensive — cannot happen with a valid token) ──
  if (!tid || !fid) {
    return new Response(
      JSON.stringify({ error: "Missing required params: tid and fid" }),
