...
export default function PricingDescription() {
  ...
  return (
    <div className="bg-[#050B14] py-24 px-6 font-sans relative overflow-hidden">
      {/* Background Ambience */}
      <div className="absolute top-0 left-1/4 w-[50vw] h-[50vw] bg-cyan-600/5 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-0 right-1/4 w-[40vw] h-[40vw] bg-fuchsia-600/5 rounded-full blur-[120px] pointer-events-none" />

      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-20">
          <div className="inline-flex items-center gap-2 bg-cyan-950/30 border border-cyan-500/30 text-cyan-400 px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.3em] mb-6 shadow-[0_0_15px_rgba(6,182,212,0.1)]">
          ...
          <h2 className="text-4xl lg:text-6xl font-black text-white mb-6 uppercase tracking-tight">
            Service <span className="text-cyan-400">Tiers</span>
          </h2>
          <p className="text-slate-500 font-medium max-w-2xl mx-auto uppercase tracking-widest text-xs">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {plans.map((plan, index) => (
            <div 
              key={index} 
              className={`relative bg-[#0A1324]/60 backdrop-blur-xl rounded-[2.5rem] p-8 border transition-all duration-500 flex flex-col group ${
                plan.isPopular 
                ? 'border-cyan-500/40 shadow-[0_0_40px_rgba(6,182,212,0.1)] scale-105 z-20' 
                : 'border-white/5 hover:border-white/20 z-10'
              }`}
            >
              {plan.isPopular && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-cyan-600 text-white text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest shadow-lg">
                  Most Popular
                </div>
              )}
              ...
              <div className="mb-8">
                ...
                <p className="text-xs text-slate-500 mt-2 font-bold uppercase tracking-wider">{plan.description}</p>
              </div>

              <div className="mb-10">
                <span className="text-4xl font-black text-white tracking-tighter">{plan.price}</span>
                <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest ml-2">/ Protocol</span>
              </div>

              <ul className="space-y-5 mb-10 flex-grow">
                {plan.features.map((feature, idx) => (
                  <li key={idx} className="flex items-start gap-3 text-xs font-bold text-slate-400 uppercase tracking-wide">
                    <Check className={`w-4 h-4 shrink-0 ${plan.isPopular ? 'text-cyan-400' : 'text-slate-600'}`} />
                    <span>{feature}</span>
                  </li>
                ))}
              </ul>
