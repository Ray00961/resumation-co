...
export default function CareerAnalysis() {
  ...
  return (
    <div className="min-h-screen bg-cyber-bg p-4 md:p-6 flex flex-col items-center font-sans text-cyber-text relative overflow-hidden">
      <div className="absolute top-0 right-0 w-[60vw] h-[60vw] bg-cyber-teal/5 rounded-full blur-[150px] pointer-events-none" />
      ...
      <div className="max-w-5xl w-full flex-1 flex flex-col h-[90vh] relative z-10">
        <div className="flex items-center justify-between mb-6 shrink-0 border-b border-white/5 pb-4">
          <button onClick={() => navigate('/dashboard')} className="flex items-center text-cyber-dim hover:text-cyber-cyan transition-all font-black text-[10px] uppercase tracking-widest group">
            <ArrowLeft className="w-4 h-4 mr-2 group-hover:-translate-x-1 transition-transform" /> Exit Node
          </button>
          <div className="bg-[rgba(35,113,123,0.12)] backdrop-blur-md px-4 py-2 rounded-xl border border-cyber-teal/20 flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-cyber-cyan shadow-[0_0_15px_rgba(13,138,158,0.1)]">
            <BrainCircuit className="w-4 h-4 animate-pulse" /> Neural AI Analyzer
          </div>
        </div>

        <div className="bg-[rgba(35,113,123,0.08)] backdrop-blur-2xl rounded-[2.5rem] border border-white/5 flex flex-1 overflow-hidden shadow-2xl relative">
          <div className="flex-1 flex flex-col">
            <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-hide">
              {messages.map((msg) => (
                <div key={msg.id} className={`flex gap-4 ${msg.type === 'user' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border transition-all ${msg.type === 'user' ? 'bg-cyber-teal/20 border-cyber-teal/30 text-cyber-cyan' : 'bg-white/5 border-white/10 text-cyber-cyan'}`}>
                  ...
                  <div className={`flex flex-col ${msg.type === 'user' ? 'items-end' : 'items-start'} max-w-[85%]`}>
                    <div className={`rounded-2xl p-5 shadow-xl whitespace-pre-wrap text-sm md:text-base leading-relaxed border ${msg.type === 'user' ? 'bg-cyber-teal/10 border-cyber-teal/20 text-slate-200 rounded-tr-none' : 'bg-white/5 border-white/10 text-slate-300 rounded-tl-none'}`}>
                      {msg.hasAttachment && <div className="text-[9px] font-black uppercase mb-3 text-cyber-cyan tracking-widest flex items-center gap-1"><Paperclip className="w-3 h-3" /> Data Stream Attached</div>}
                    ...
                    {msg.isAction && !isDecisionMade && showRewriteOption && (
                      <div className="flex gap-2 mt-4 animate-in slide-in-from-top-2">
                        <button onClick={() => handleRewriteDecision(true)} className="flex items-center gap-2 px-6 py-2.5 bg-cyber-teal text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-cyber-cyan shadow-lg transition-all active:scale-95"><ThumbsUp className="w-3 h-3" /> Initialize Rewrite</button>
                        <button onClick={() => handleRewriteDecision(false)} className="flex items-center gap-2 px-6 py-2.5 bg-white/5 text-cyber-dim rounded-xl font-black text-[10px] uppercase tracking-widest hover:text-white transition-all border border-white/5"><ThumbsDown className="w-3 h-3" /> Decline</button>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {isDecisionMade && showRewriteOption && (
                <div className="max-w-sm mx-auto bg-[rgba(35,113,123,0.15)] backdrop-blur-3xl rounded-[2.5rem] shadow-2xl animate-in slide-in-from-bottom-6 duration-700 mt-8 mb-8 border border-cyber-teal/20 p-1">
                  <div className="bg-[rgba(31,43,45,0.8)] p-8 rounded-[2.4rem] text-center relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-cyber-cyan to-transparent" />
                    <div className="bg-cyber-teal/10 text-cyber-cyan px-4 py-1 rounded-full text-[9px] font-black uppercase tracking-widest mb-6 inline-block border border-cyber-teal/20">Upgrade Protocol</div>
                    ...
                    <div className="my-8 text-white bg-white/5 py-6 rounded-3xl border border-white/5 shadow-inner">
                      <span className="text-5xl font-black italic tracking-tighter text-cyber-cyan">250</span>
                      <span className="text-[10px] font-black opacity-40 uppercase tracking-widest ml-2">EGP</span>
                    </div>
                    <button onClick={handleAnalysisPlanPayment} className="w-full py-5 bg-cyber-teal text-white font-black rounded-2xl hover:bg-cyber-cyan transition-all shadow-[0_0_20px_rgba(13,138,158,0.3)] flex justify-center items-center gap-3 uppercase text-[10px] tracking-[0.2em]">
              ...
              {isTyping && (
                <div className="flex gap-4 animate-pulse">
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-5 shadow-sm flex items-center gap-3">
                    ...
                    <span className="text-cyber-dim text-[10px] font-black uppercase tracking-widest italic">Analyzing Career Matrix...</span>
                  </div>
                </div>
              )}
            ...
            <div className="p-6 bg-[rgba(31,43,45,0.4)] border-t border-white/5 shrink-0">
              {selectedFile && (
                <div className="mb-4 px-5 py-3 bg-cyber-teal/5 text-cyber-cyan rounded-xl text-[10px] flex items-center justify-between font-black uppercase tracking-[0.2em] border border-cyber-teal/20 shadow-[0_0_15px_rgba(13,138,158,0.05)]">
              ...
              <div className="flex gap-3 items-end bg-[rgba(31,43,45,0.95)] p-2 rounded-[2rem] border border-white/5 focus-within:border-cyber-teal/30 transition-all">
                <label className="cursor-pointer p-4 text-cyber-dim hover:text-cyber-cyan transition-all">
                ...
                <textarea
                  ...
                  className="flex-1 bg-transparent border-none p-4 focus:outline-none resize-none max-h-32 text-sm font-medium text-slate-200 placeholder-cyber-dim"
                  rows={1}
                />
                <button
                  onClick={handleSendMessage}
                  disabled={(!inputValue.trim() && !selectedFile) || isTyping}
                  className="bg-cyber-teal text-white p-5 rounded-[1.5rem] hover:bg-cyber-cyan shadow-[0_0_20px_rgba(13,138,158,0.2)] active:scale-95 transition-all disabled:opacity-20"
                >
