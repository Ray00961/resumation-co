...
import { Copy, Check, Share2, MessageCircle, Zap, Crown, Gift } from "lucide-react";
...
export default function PromoCodeBanner({ promoCode, promoExpiresAt, isFounder }: PromoCodeBannerProps) {
  const { lang, isRtl } = useLang();

  const [timeLeft, setTimeLeft] = useState<TimeLeft | null>(() => calcTimeLeft(promoExpiresAt));
  ...
  const urgency = isUrgent
    ? { text: "#f87171",  border: "rgba(248,113,113,0.35)",  glow: "rgba(248,113,113,0.08)",  pulse: true  }
    : isWarning
    ? { text: "#fbbf24",  border: "rgba(251,191,36,0.30)",   glow: "rgba(251,191,36,0.07)",   pulse: false }
    : { text: "#E0C58F",  border: "rgba(224,197,143,0.25)",  glow: "rgba(224,197,143,0.06)",  pulse: false };
  ...
  const handleShare = async () => {
    if (navigator.share) {
      try {
        await navigator.share({ title: "Resumation.co — 50% Off", text: shareText, url: siteUrl });
        return;
      } catch {}
    }
  ...
  return (
    <div
      ...
      className="relative rounded-[2rem] mb-8 overflow-hidden"
    >
      {/* Outer glow ring */}
      <div
        className="absolute inset-0 rounded-[2rem] pointer-events-none"
        style={{ boxShadow: `0 0 60px ${urgency.glow}, 0 0 0 1px ${urgency.border}` }}
      />
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2.5 mb-4 flex-wrap">
              <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0"
                style={{ background: `${urgency.glow}`, border: `1px solid ${urgency.border}` }}>
              ...
              {isFounder && (
                <span
                  ...
                  style={{ background: "rgba(224,197,143,0.10)", border: "1px solid rgba(224,197,143,0.25)", color: "#E0C58F" }}
                >
            ...
            >
              ...
              <div className="space-y-1.5">
                {t.rewardRows.map((row) => (
                  <div key={row.plan} className="flex items-center justify-between gap-4">
                    <span className="text-[11px] text-[#7A8FAA] font-medium">{row.plan}</span>
                    <span className="text-[11px] font-black text-amber-400 flex-shrink-0">{row.coins}</span>
                  </div>
                ))}
              </div>
