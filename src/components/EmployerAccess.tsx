...
export default function EmployerAccess() {
  ...
  return (
    <section id="exclusive-access" className="py-24 px-6 lg:px-16 bg-cyber-bg font-sans relative overflow-hidden">
      
      {/* Background Ambience */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60vw] h-[60vw] bg-cyber-teal/5 rounded-full blur-[150px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="relative">
          ...
          {/* Cyber Lock Overlay */}
          {!loading && !isUnlocked && (
            <div className="absolute inset-0 z-30 backdrop-blur-xl bg-cyber-bg/40 flex flex-col items-center justify-center rounded-[3rem] border border-white/5 shadow-2xl transition-all duration-700">
              <div className="bg-[rgba(35,113,123,0.15)] p-10 rounded-[2.5rem] shadow-2xl text-center border border-white/10 max-w-md mx-6 relative overflow-hidden">
                ...
                <p className="text-cyber-muted mb-8 text-sm leading-relaxed font-medium">
          ...
          <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 transition-all duration-700 ${(!isUnlocked || loading) ? 'blur-md opacity-30 pointer-events-none select-none grayscale' : ''}`}>
            {employersList.map((employer, index) => (
              <motion.a
                ...
                className="bg-[rgba(35,113,123,0.12)] backdrop-blur-md rounded-3xl p-6 border border-white/5 flex items-center gap-5 hover:border-cyber-teal/30 hover:bg-[rgba(35,113,123,0.18)] transition-all group relative overflow-hidden"
              >
                ...
                <div className="flex flex-col">
                  <h3 className="text-lg font-black text-white tracking-tight group-hover:text-cyber-cyan transition-colors">
                    {employer.name}
                  </h3>
